...
// import { Settings, Users, Link2, Flag } from 'lucide-react';
// import { FiEdit2, FiTrash2 } from 'react-icons/fi';

// export default function Setup() {
//   const [activeTab, setActiveTab] = useState<
//     'Role Management' | 'Staff Management' | 'Lead Sources' | 'Lead Status' | 'Kanban Status'
//   >('Role Management');
...
//       }
//     }
//     if (leadStatuses.length > 0 && kanbanStatusNames.length === 0) {
//       setKanbanStatusNames(leadStatuses.map((s) => s.name));
//     }
//   }, [leadStatuses, kanbanStatusNames.length]);
...
//       });
//   };
//   const openEditSource = (item: Item) => {
//     setEditingSource(item);
//     setEditSourceName(item.name);
//     setEditSourceOrder(item.order);
...
//       });
//   };
//   const deleteSource = (item: Item) => {
//     const ok = typeof window !== 'undefined' ? window.confirm('Delete this source?') : true;
//     if (!ok) return;
//     axios
//       .delete(`${baseUrl.leadSources}/${encodeURIComponent(item.name)}`, {
//         headers: token ? { Authorization: `Bearer ${token}` } : undefined,
//       })
//       .then(() => axios.get(baseUrl.leadSources, { headers: token ? { Authorization: `Bearer ${token}` } : undefined }))
//       .then((res) => {
...
//         setLeadSources((prev) => prev.filter((x) => x.name !== item.name));
...
//       });
//   };
//   const openEditStatus = (item: Item) => {
//     setEditingStatus(item);
//     setEditStatusName(item.name);
//     setEditStatusOrder(item.order);
...
//       });
//   };
//   const deleteStatus = (item: Item) => {
//     const ok = typeof window !== 'undefined' ? window.confirm('Delete this status?') : true;
//     if (!ok) return;

//     axios
//       .delete(`${baseUrl.leadStatuses}/${encodeURIComponent(item.name)}`, {
//         headers: token ? { Authorization: `Bearer ${token}` } : undefined,
//       })
//       .then(() =>
//         axios.get(baseUrl.leadStatuses, {
//           headers: token ? { Authorization: `Bearer ${token}` } : undefined,
//         })
//       )
//       .then((res) => {
//         setLeadStatuses(
//           parseList(res.data?.data ?? res.data).sort((a, b) => a.order - b.order)
//         );
//         toast.success('Lead status deleted successfully');
//       })
//       .catch((error) => {
//         console.error('Failed to delete lead status', error);
//         setLeadStatuses((prev) => prev.filter((x) => x.name !== item.name));
//         toast.error(error?.response?.data?.message || 'Failed to delete lead status');
//       });
//   };

//   if (!loadingPermissions && (!setupPermissions || !setupPermissions.readAll)) {
//     return (
//       <Layout label="Setup">
//         <div className="p-6">
...
//               {setupPermissions?.readAll && (
//                 <button
//                   onClick={() => setActiveTab('Role Management')}
//                   className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium ${activeTab === 'Role Management'
...
//                   className={`mt-1 flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium ${activeTab === 'Staff Management'
//                       ? 'bg-blue-50 text-blue-700'
//                       : 'text-gray-700 hover:bg-gray-50'
//                     }`}
//                 >
...
//                   className={`mt-1 flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium ${activeTab === 'Lead Sources'
//                       ? 'bg-blue-50 text-blue-700'
//                       : 'text-gray-700 hover:bg-gray-50'
//                     }`}
//                 >
...
//                   className={`mt-1 flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium ${activeTab === 'Lead Status'
//                       ? 'bg-blue-50 text-blue-700'
//                       : 'text-gray-700 hover:bg-gray-50'
//                     }`}
//                 >
...
//                   className={`mt-1 flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium ${activeTab === 'Kanban Status'
//                       ? 'bg-blue-50 text-blue-700'
//                       : 'text-gray-700 hover:bg-gray-50'
//                     }`}
//                 >
...
//                       <button
//                         type="button"
//                         onClick={() => setKanbanStatusNames(leadStatuses.map((s) => s.name))}
...
//                           const checked = kanbanStatusNames.includes(status.name);
//                           return (
//                             <label
//                               key={status.name}
...
//                                     if (isChecked) {
//                                       if (prev.includes(status.name)) return prev;
//                                       return [...prev, status.name];
//                                     }
//                                     return prev.filter((n) => n !== status.name);
//                                   });
//                                 }}
